    ...
    : ControllerBase
{
    ...
    )
    {
        ...
        try
        {
            ...
            if (payload.Data is not JArray dataArray)
                throw new InvalidOperationException("Payload data must be a JSON array.");
        ...
        catch (Exception e)
        {
            ...
            _ = new LogDataModel
            {
                ...
                ProcessType = LogDataModel.ProcessKind.ML_INSERTDATA,
                ProcessIndex = 1,
                TotalProcess = payload.Data is JArray arr ? arr.Count : 0,
                Log = e.Message,
                Request = payload.Data.ToString(),
                Response = null,
            }.Save(pgHelper: pgHelper, cancellationToken: cancellationToken);
